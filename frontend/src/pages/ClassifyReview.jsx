...
function ClassifyReview() {
  ...
  const { candidates, fault_type_proposal, severity_proposal, is_repeat_caller, existing_ticket_numbers, intake_id } = intakeResponse;
  ...
  async function handleConfirm() {
    ...
    try {
      const payload = {
        intake_id:              intake_id,
        confirmed_app_id:       noMatch ? null : selectedAppId,
        ...
        predicted_app_id:       candidates.find(c => c.is_primary)?.application_id ?? null,
    ...
    <div style={{ maxWidth: '700px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      {is_repeat_caller && (
        <div style={{ background: '#FAEEDA', border: '0.5px solid #FAC775', borderRadius: '10px', padding: '12px 16px', fontSize: '13px', color: '#633806' }}>
          ⚠ Repeat caller — existing ticket(s): <strong>{(existing_ticket_numbers || []).join(', ')}</strong>
        </div>
      )}
      ...
      <div style={card}>
        ...
        {candidates.map(c => (
          <div key={c.application_id} onClick={() => { setSelectedAppId(c.application_id); setNoMatch(false); }}
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '13px', fontWeight: 500 }}>{c.application_name}</div>
